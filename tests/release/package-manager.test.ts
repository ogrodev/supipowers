...
} from "../../src/release/package-manager.js";
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "supi-release-pm-"));
});
...
describe("detectPackageManager", () => {
  ...
  test("falls back to lockfiles in the configured precedence order", () => {
  ...
  test("preserves current single-package behavior by defaulting to bun", () => {
    writePackageJson({ name: "repo", version: "1.0.0" });

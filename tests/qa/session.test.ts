...
describe("E2E QA session lifecycle", () => {
  ...
  test("createNewE2eSession initializes all phases as pending", () => {
    const ledger = createNewE2eSession(tmpDir, DEFAULT_E2E_QA_CONFIG);
  ...
  test("advanceE2ePhase updates status and timestamps", () => {
    const ledger = createNewE2eSession(tmpDir, DEFAULT_E2E_QA_CONFIG);
    const updated = advanceE2ePhase(tmpDir, ledger, "flow-discovery", "running");
    ...
    const completed = advanceE2ePhase(tmpDir, updated, "flow-discovery", "completed");
  ...
  test("advanceE2ePhase persists changes to disk", () => {
    const ledger = createNewE2eSession(tmpDir, DEFAULT_E2E_QA_CONFIG);
    advanceE2ePhase(tmpDir, ledger, "flow-discovery", "completed");

...
- Refresh `parseSystemPrompt` / `parseIndividualSkills` (`src/context/analyzer.ts`) for OMP 14.9.3's new `<|START_PROJECT|>` / `<|START_ENV|>` / `<|START_CONTRACT|>` prompt markers and bullet-list `# Skills` block. `/supi:context` and `/supi:optimize-context` now label project context, environment, and contract sections again, and per-skill token accounting works against real OMP prompts (the previous bound let the Skills block bleed past sibling h2 headings and misidentified `## MCP Server Instructions` as a skill). The legacy `<project>` / `## skill-name` XML matchers remain as fallbacks for older OMP versions and synthetic test inputs.
- Drop the unreachable `case "notebook":` arm in `getUiDesignWritePaths` (`src/ui-design/session.ts`). OMP 14.7.4 retired the dedicated `notebook` tool; `.ipynb` writes now flow through the existing `edit` arm via OMP's notebook-serialization helpers.
- OMP 14.9.3 → 14.9.8 audit follow-up. Refresh the `planning_ask` registration comment (`src/planning/planning-ask-tool.ts`) for OMP 14.9.5's new `ask.timeout` default of `0`; the tool itself is unchanged and remains necessary for user-configured non-zero timeouts, the `recordUiDesignReviewApproval` side effect, and the `ask`-guard redirect. Document the OMP 14.9.5 cross-session `agent://` / `artifact://` / `memory://` resolution (`docs/supipowers/ultraplan-authoring.md`) — the contract now used by ultraplan scenario proofs. Per-agent async-job ownership (OMP 14.9.5) and the new `:conflicts` selector + `conflict://` writes (OMP 14.9.8) land transparently for sub-agents spawned through `createAgentSession`; an explicit `merge-blocked` prompt hint in `src/ultraplan/batch/merge.ts` is deferred until we observe an agent stalling on a real merge conflict. Audit baseline advanced to 14.9.8 in `.omp/omp-audit-config.json`.
- Remove the `/supi:mcp` command and the supipowers MCP server management feature. The `mcpc` integration, the `mcpc_manager` agent tool, the registry under `.omp/supipowers/.mcp.json`, the `$tag`-driven gateway activation, the `mcp.closeSessionsOnExit` config block, and the `mcpc` optional dependency are all gone. Use OMP's native MCP host configuration (`agent/mcp.json`) for MCP servers; the legacy context-mode cleanup that strips obsolete `context-mode` / `supi-context-mode` entries from `agent/mcp.json` is preserved.

### Improvements

- `/supi:harness pr-comment` — render or post a sticky GitHub PR comment from the latest `validate-report.json`. One comment per PR (located by an `<!-- supipowers:harness:v1 ... -->` marker), edited in place on every push. Default cadence is `every-push`; `on-status-change` mode skips the upsert when status hasn't flipped. Fail-open: missing `gh`, missing auth, or missing PR context all fall back to `$GITHUB_STEP_SUMMARY` and exit 0. New config block in `HarnessCiConfig.prComment` (in the design spec). The `ci-local-wiring` validate check now also warns when the workflow lacks `pull-requests: write` and `prComment.enabled` is true. Local preview: `bunx supipowers harness pr-comment --dry-run`. See `docs/supipowers/harness/pr-comment.md`.
- `/supi:harness docs` — new 6.5th stage (between Implement and Validate). In `extensive` mode it fans out one subagent per architectural layer to produce a 150-LOC, agent-only knowledge document at `docs/layers/<id>.md` plus a deterministic `docs/README.md` index. The first 30 LOC of each per-layer doc (`## Agent context`) is the addendum the `layer-context-inject` hook injects on every covered edit, replacing the previous architecture-table-derived fallback when a per-layer doc exists. Default tier is `simple` (the stage is a no-op); the user picks the tier at the end of the Design gate. Provenance markers and source-hash composition catch user edits (preserved) and stale docs (regenerated). The Validate stage gains a `docs-validation` check covering doc-validator re-run, index ↔ filesystem consistency, hook integration smoke test, and sourceHash drift (toggle via `harness.docs.drift_warning.enabled`). See `docs/supipowers/harness.md#per-layer-agent-docs-docs-stage`.
...
- Recommend OMP ≥14.7.8 to avoid the large-repo startup hang fixed in oh-my-pi#975 (workspace discovery used to enumerate every gitignored path before the agent could send its first message). OMP ≥14.9.2 additionally consolidates workspace discovery into a single native `listWorkspace` walk and now correctly includes top-level gitignored `AGENTS.md` files in directory context. supipowers' headless `createAgentSession` callers (review, ultraplan, harness, fix-pr, docs/drift, quality gates, ui-design) inherit these robustness improvements transparently — no code change is required, but bumping to OMP ≥14.9.2 is strongly recommended for any project with a large `.gitignore` footprint.

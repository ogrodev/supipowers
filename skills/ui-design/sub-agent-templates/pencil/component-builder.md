...
2. Use `mcp_pencil_batch_design` to insert a single reusable frame under `parentNodeId` named exactly `componentSpec.name`. The frame MUST set `reusable: true` so the Director can instantiate it later via a `ref`.
